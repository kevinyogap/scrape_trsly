...
class TrstdlyScraper:
    ...
    def _extract_raw_content(self, soup: BeautifulSoup) -> str:
        ...
        unwanted_selectors = ['nav[aria-label="breadcrumb"]', '.article-share', '.gpt-ads', '.banner', 'script', 'style', 'h1.article-title', 'p.article-sinopsis', 'time', '.article-tag', '.flex.flex-wrap.justify-between']
        for selector in unwanted_selectors:
            for elem in content_copy.select(selector):
                elem.decompose()
        for figure in content_copy.find_all('figure'):
            img = figure.find('img')
            if not img:
                figure.decompose()
                continue
        ...
        for tag in content_copy.find_all(['li', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ol', 'ul', 'blockquote']):
            tag.attrs = {}
    ...
    # --- FUNGSI BARU ---
    def _separate_cover_and_content(self, raw_content_html: str) -> Tuple[Optional[str], str]:
        """
        Memisahkan gambar cover dari sisa konten.
        Menghapus figcaption yang berasosiasi dengan gambar cover.
        """
        if not raw_content_html:
            return None, ""
        
        # Menggunakan wrapper div agar parsing konsisten
        soup = BeautifulSoup(f"<div>{raw_content_html}</div>", 'lxml')
        
        cover_img_tag = soup.find('img')
        cover_image_html = None

        if cover_img_tag:
            cover_image_html = str(cover_img_tag)
            
            # Cari dan hapus figcaption yang mengikuti gambar
            next_element = cover_img_tag.find_next_sibling()
            # Lewati node teks kosong (seperti newline)
            while next_element and isinstance(next_element, str) and not next_element.strip():
                next_element = next_element.find_next_sibling()
            
            if next_element and next_element.name == 'figcaption':
                next_element.decompose()
            
            # Hapus gambar cover dari soup
            cover_img_tag.decompose()

        # Dapatkan sisa konten dari dalam wrapper div
        remaining_content_html = soup.div.decode_contents()
        
        return cover_image_html, remaining_content_html

    # --- FUNGSI DIMODIFIKASI ---
    def create_clean_html_for_file(self, article_data: Dict[str, any]) -> str:
        """
        Membuat HTML bersih untuk file dengan format yang diminta:
        meta -> cover image -> h1 -> description -> rest of content.
        """
        html_parts = []
        # 1. Tambah semua meta tag
        if article_data['title']: html_parts.append(f'<title>{article_data["title"]}</title>')
        if article_data['meta_description']: html_parts.append(f'<meta name="description" content="{article_data["meta_description"]}">')
        if article_data['meta_keywords']: html_parts.append(f'<meta name="keywords" content="{article_data["meta_keywords"]}">')
        if article_data['meta_tag']: html_parts.append(f'<meta name="tags" content="{article_data["meta_tag"]}">')
        ld_meta_tags = self.generate_meta_tags(article_data)
        if ld_meta_tags: html_parts.append(ld_meta_tags)
        html_parts.append('')

        # 2. Tambah cover image (jika ada)
        if article_data.get('cover_image'):
            html_parts.append(article_data['cover_image'])
            html_parts.append('')

        # 3. Tambah H1 dan deskripsi
        if article_data['title']: html_parts.extend([f'<h1>{article_data["title"]}</h1>', ''])
        if article_data['description']: html_parts.extend([f'<p>{article_data["description"]}</p>', ''])

        # 4. Tambah sisa konten yang sudah bersih
        # `remaining_content` sudah dipisahkan di `scrape_article`
        clean_content = self._post_process_content(article_data.get('remaining_content', ''))
        if clean_content:
    ...
    def scrape_article(self, url: str) -> Optional[Tuple[Dict, str, str]]:
        ...
        # Pisahkan gambar cover dari sisa konten
        cover_image_html, remaining_body_content = self._separate_cover_and_content(article_data['content'])
        
        # Simpan bagian-bagian ini ke dalam dict untuk digunakan nanti
        article_data['cover_image'] = cover_image_html
        article_data['remaining_content'] = remaining_body_content
        
        # (A) Buat HTML LENGKAP untuk disimpan ke FILE
        #    Fungsi ini sekarang menggunakan 'cover_image' dan 'remaining_content'
        full_html_for_file = self.create_clean_html_for_file(article_data)

        # (B) Buat KONTEN BERSIH untuk disimpan ke DATABASE
        #    Format: cover_image -> h1 -> description -> sisa konten
        db_parts = []
        if cover_image_html:
            db_parts.append(cover_image_html)
            db_parts.append('')
        ...
        clean_remaining_content = self._post_process_content(remaining_body_content)
        db_parts.append(clean_remaining_content)
        
        content_for_db = '\n'.join(db_parts)
        content_for_db = re.sub(r'\n{3,}', '\n\n', content_for_db).strip()
...
# --- CONTOH PENGGUNAAN (Tidak Berubah) ---
