...
def truncate_text_by_word(text, max_length=250):
    """
    Memotong teks hingga di bawah max_length dengan menghapus kata-kata utuh dari belakang.
    Jika kata pertama saja sudah lebih panjang dari max_length, maka akan dipotong paksa.
    """
    if len(text) <= max_length:
        return text

    truncated = text
    while len(truncated) > max_length:
        # Temukan spasi terakhir untuk memotong dengan aman
        last_space = truncated.rfind(' ')
        if last_space == -1:
            # Tidak ada spasi, berarti hanya satu kata yang sangat panjang.
            # Lakukan pemotongan paksa sebagai fallback.
            return text[:max_length]
        
        # Potong teks pada spasi terakhir
        truncated = truncated[:last_space]
            
    return truncated.strip()


def truncate_alt_attributes_by_word(html_content):
    """
    ...
    # Pola regex untuk menemukan alt="teks" atau alt='teks'
    pattern = re.compile(r'alt=(["\'])(.*?)\1', re.IGNORECASE | re.DOTALL)

    def replacer(match):
        quote_char = match.group(1)  # Menangkap tanda kutip (' atau ")
        alt_text = match.group(2)    # Menangkap teks alt asli
        
        # Gunakan fungsi baru untuk memotong teks secara cerdas
        truncated_text = truncate_text_by_word(alt_text, 250)
        
        # Bangun kembali atribut alt
...
def process_content(original_content, meta_description, meta_tags, meta_keywords):
    ...
    """
    # Proses konten untuk memotong semua atribut alt yang terlalu panjang
    original_content = truncate_alt_attributes_by_word(original_content)
...
def convert_json_to_wordpress_xml(json_data, output_file):
    ...
    """
    try:
        # Penyesuaian untuk JSON baru: mengambil list dari kunci 'scraped_articles'
        if 'scraped_articles' not in json_data or not isinstance(json_data['scraped_articles'], list):
            raise ValueError("Invalid JSON structure: 'scraped_articles' array not found")
        
        articles = json_data['scraped_articles']
        xml_parts = []
